...
const staticCopy = {
  ...
  madre: 'MADRE is the living structural core of VEHICLE Systems Lab.\n\nNot a product. The mother architecture.\n\nCurrently in active training and research consolidation.\n\nVisit the MADRE page to interact with the core, explore the architecture, and learn about the Technology Alliance.\n\nDocuments available below.'
};
...
function startMadreBreathAudio() {
  ...
  try {
    ...
    breathTimer = window.setInterval(() => {
      ...
      const breathScale = 0.96 + inhale * 0.20 + (micro - 0.5) * 0.012;
      const glowIntensity = Math.round(82 + inhale * 108);
      const glowAlpha = (0.88 + inhale * 0.12).toFixed(2);
      core.style.transform = `scale(${breathScale.toFixed(4)})`;
      core.style.filter = `drop-shadow(0 0 ${glowIntensity}px rgba(142, 236, 255, ${glowAlpha}))`;
    }, 70);
  } catch (error) {
...
function shouldIgnoreWakeTarget(target) {
  return Boolean(target?.closest('a, .project-name, [data-back], .scene-downloads'));
}

function handleWakeInteraction(event) {
  if (currentState !== 'sleeping') return;
  if (shouldIgnoreWakeTarget(event.target)) return;
  wakeMadre();
}
...
[stage, coreWrap, core, sleepLabel].forEach((target) => {
  if (!target) return;
  target.addEventListener('click', handleWakeInteraction);
});
