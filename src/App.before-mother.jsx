...
function MotherHero({ active, activeKey, onSelect, onOpenSupra, onOpenOdi, onOpenCps }) {
  ...
  const replyFor = (text) => {
    ...
    if (lower.includes("supra")) {
      return "VEHICLE-SUPRA watches coherence, tension, recovery, and continuity inside autonomous agent networks.";
    }

    if (lower.includes("cps") || lower.includes("civil") || lower.includes("protection")) {
      return "VEHICLE-CPS turns civil protection into a structured human field: regimes, supervision, risk, and recovery.";
    }

    if (lower.includes("odi") || lower.includes("space") || lower.includes("debris")) {
      return "VEHICLE-ODI reads orbital debris as a relational field of risk, priority, and intervention.";
    }

    if (lower.includes("contact") || lower.includes("collab") || lower.includes("partner")) {
      return "For collaboration, the institutional contact is contact@vehiclesystemslab.com.";
    }

    if (lower.includes("vehicle")) {
      return "VEHICLE studies relational systems, structural coherence, contraction, tension, and stability across connected domains.";
    }

    return "Ask me about VEHICLE, SUPRA, CPS, ODI, publications, or collaboration.";
  };
  ...
  const handleSubmit = (event) => {
    ...
    if (!clean) return;
    const nextReply = replyFor(clean);
    setMessages((current) => [
      ...current,
      { role: "human", text: clean },
      { role: "mother", text: nextReply },
    ]);
    setInput("");
  };

  return (
    <div className={`mother-home-card mother-home-${activeKey}`}>
      <div className="mother-field" aria-hidden="true">
        <div className="mother-halo mother-halo-one" />
        <div className="mother-halo mother-halo-two" />
        <div className="mother-halo mother-halo-three" />
        <div className="mother-orbit mother-orbit-one" />
        <div className="mother-orbit mother-orbit-two" />
        <div className="mother-orbit mother-orbit-three" />
        <div className="mother-core-live">
          <div className="mother-core-pulse" />
          <div className="mother-core-eye" />
          <div className="mother-core-seed" />
        </div>
        <span className="mother-signal signal-one" />
        <span className="mother-signal signal-two" />
        <span className="mother-signal signal-three" />
      </div>

      <div className="mother-interface-layer">
        <div className="mother-nameplate">
          <span />
        ...
        <button
          type="button"
          className="mother-open-dialogue"
          onClick={() => setDialogOpen((value) => !value)}
          aria-expanded={dialogOpen}
        >
          {dialogOpen ? "Close dialogue" : "Speak with Mother"}
        </button>
      </div>

      {dialogOpen && (
        <div className="mother-dialogue-panel" aria-live="polite">
          <div className="mother-dialogue-stream">
            {messages.map((message, index) => (
              <article key={`${message.role}-${index}`} className={`mother-message ${message.role}`}>
                <span>{message.role === "mother" ? "Mother" : "You"}</span>
                <p>{message.text}</p>
              </article>
            ))}
          </div>

          <form className="mother-dialogue-form" onSubmit={handleSubmit}>
            <input
              type="text"
              value={input}
              onChange={(event) => setInput(event.target.value)}
              placeholder="Ask Mother..."
              aria-label="Ask Mother"
            />
            <button type="submit">Send</button>
          </form>

          <div className="mother-dialogue-actions">
            <button type="button" onClick={onOpenSupra}>SUPRA</button>
            <button type="button" onClick={onOpenOdi}>ODI</button>
            <button type="button" onClick={onOpenCps}>CPS</button>
            <button type="button" onClick={() => onSelect("partnerships")}>Contact</button>
          </div>
        </div>
      )}
    </div>
    ...
    <main className="page-shell">
      <section className="hero-section">
        ...
        <div className="hero-content">
          ...
          <div className="right-panel mother-right-panel">
            <MotherHero
              active={active}
              activeKey={activeKey}
              onSelect={setActiveKey}
              onOpenSupra={() => setView("supra")}
              onOpenOdi={() => setView("odi")}
              onOpenCps={() => setView("cps")}
            />
